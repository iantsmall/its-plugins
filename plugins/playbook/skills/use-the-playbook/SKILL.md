---
name: use-the-playbook
description: This skill should be used when a session references "the playbook", "the Marquet playbook", "the six plays", "Leadership Is Language", "Turn the Ship Around", "red work" or "blue work" in general terms, or asks what this framework is or how it's organized. It also functions as a self-triggering index — read it to decide which sibling skill (control-the-clock, collaborate-not-coerce, commit-not-comply, complete-not-continue, improve-not-prove, connect-not-conform, intent-language, speak-plainly) applies to the current moment, even when the user never names a play explicitly. Do NOT use this in place of a specific play skill once the relevant play is identified — invoke that one instead, since it carries the actual guidance.
---

# Using the playbook: overview and self-trigger index

This plugin packages L. David Marquet's communication framework — six plays from *Leadership Is Language* plus intent-based language from *Turn the Ship Around* — as one skill per play so each loads only when it's actually relevant, rather than bundling all seven into one large file. It also carries one skill that is **not** from Marquet's framework: `speak-plainly`, a separate personal practice about plain, accessible output, grouped here because it's the same kind of thing (how to communicate), not because it shares a source. That's also why the plugin itself is just called `playbook`, not `marquet-playbook` — it's a personal working-practice playbook *based on* Marquet, not a packaging of his framework alone.

**Naming note:** these eight skills use short, bare names (`control-the-clock`, not `playbook-control-the-clock`) rather than prefixing every one with the plugin's own name — a deliberate choice, since these are invoked by name in everyday conversation far more than a typical workflow skill, and the shorter form reads better in that context. Worth revisiting if it ever collides with a skill from another installed plugin.
...
| The user signals fatigue or overload, or a response is about to lean on jargon, a wall of prose, or more decision options than the moment needs | `speak-plainly` |
